...
- **Lock and unlock workspace:** — Allows users to manually lock the workspace to temporarily prevent runs. When a workspace's execution mode is set to "local", this permission is required for performing local CLI runs using this workspace's state. Please keep in mind that even though this is a team-based permission, once a user sets a lock on a workspace, only that user can unlock it.
...
- Apply runs
- Lock and unlock workspace
"Typically, only the user who created the lock can unlock a workspace. Administrators and members of the "Owners" team can use Force unlock to override this an unlock a workspace."
- Download Sentinel mocks
- Read and write variables
- Read and write state versions
