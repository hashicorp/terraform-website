...
To disable remote execution for a workspace, change its execution mode to "Local". This mode lets you perform Terraform runs locally with the [CLI-driven run workflow](/docs/cloud/run/cli.html). The workspace will store state and variables, which Terraform can access with the [CLI integration](/docs/cli/cloud/index.html).
