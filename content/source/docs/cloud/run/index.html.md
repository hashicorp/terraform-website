...
In more abstract terms, runs can be initiated by VCS webhooks, the manual "Start new run" action in the workspace actions menu, the standard `terraform apply` command (with the CLI integration configured), and [the Runs API](../api/run.html) (or any tool that uses that API).
