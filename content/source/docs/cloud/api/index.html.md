...
- `state-storage` — Allows an organization to store state versions in its workspaces, which enables local Terraform runs with the CLI workflow. Affects the [state versions][] endpoints.
