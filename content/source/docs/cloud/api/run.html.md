...
`data.attributes.refresh-only`                     | bool                | false                                              | Specifies whether this run should use the refresh-only plan mode, which will refresh the state without modifying any resources. Mutually exclusive with `is-destroy`.
...
`data.attributes.variables`                        | array[{key, value}] | empty                                              | Specifies an optional list of run-specific variable values. Refer to [Run-Specific Variables](https://www.terraform.io/docs/cloud/workspaces/managing-variables.html#run-specific-variables) in the Terraform Cloud documentation for more information.
