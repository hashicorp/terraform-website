...
`data.attributes.default-remote-state-access`    | bool     | `true`                   | When set to `true`, new workspaces will default to allowing remote state access to them. To learn more about this setting, refer to the [documentation](/docs/enterprise/admin/general.html#default-global-remote-state).
