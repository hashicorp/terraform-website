...
~> **Note:** Several services described below include the string `ptfe` as part of their names. See [here](https://www.terraform.io/docs/enterprise/index.html#note-about-product-names) for more details.
...
This section details how the key Docker containers work together to accomplish common tasks.
...
2. `ptfe_sidekiq` asynchronously pulls the job and passes the details to `ptfe_slug_ingress`.
3. `ptfe_slug_ingress` downloads your Git repository and `POST`s it to `ptfe_archivist`.
4. `ptfe_archivist` persists the repository in blob storage and `POST`s the Object ID to `ptfe_atlas`.
...
2. `ptfe_registry_api` pulls the module from `ptfe_archivist`, saves the metadata in PostgreSQL, and passes the details to the `ptfe_registry_worker`.
3. `ptfe_registry_worker` extracts the individual modules from your repository, updates the Private Registry tables in PostgreSQL, and `POST`s the results to `ptfe_atlas`.
...
1. `ptfe_atlas` receives request from `ptfe_nginx`, pushes a job to Redis for `ptfe_sidekiq`, and then responds to the original request immediately.
2. `ptfe_sidekiq` asynchronously pulls the job and passes the details to `ptfe_slug_ingress`.
3. `ptfe_slug_ingress` spawns a separate `git clone` of your Git repository and `POST`s it to `ptfe_archivist`.
4. `ptfe_archivist` persists the repository in blob storage and `POST`s the Object ID to `ptfe_atlas`.
...
2. `ptfe_sidekiq` asynchronously pulls the job and schedules a Sentinel run in `ptfe_nomad`.
3. `ptfe_nomad` runs the Sentinel job and `POST`s the result back to `ptfe_atlas`.
