...
Most requests in TFE follow a similar lifecycle as shown in the diagram below.
...
This section details how the key TFE containers work together to accomplish common tasks.
...
|  | `ptfe_backup_restore` | TFE Backup API | This container facilitates the [TFE backup function](https://www.terraform.io/docs/enterprise/admin/backup-restore.html). Also see the [TFE backup recommended pattern](https://learn.hashicorp.com/tutorials/terraform/pattern-backups?in=terraform/recommended-patterns). |
|  | `ptfe_health_check` | TFE Health Check API | Used to provide the [health check endpoint](https://www.terraform.io/docs/enterprise/admin/monitoring.html). |
|  | `ptfe_nginx` | TFE Nginx | NginX is used as a router in TFE and TFC. |
|  | `ptfe_outbound_http_proxy` | TFE Outbound HTTP Proxy | This was recently added to mitigate for server-side request forgery. |
