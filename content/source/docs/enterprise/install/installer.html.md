...
-> **Important:** Be careful when cloning an already installed Terraform Enterprise host machine. You cannot have more than one Terraform Enterprise instance connect to the same database unless the installation is uses the [Active/Active architecture](active-active.html). When cloning a Terraform Enterprise host machine, you must make sure that the `/etc/sysconfig/replicated` or `/etc/default/replicated`, and `/etc/sysconfig/replicated-operator` or `/etc/default/replicated-operator` maintain the correct private IP address and the hostname of the Terraform Enterprise host machine before the restoration from the image. You should also restart the `replicated` and `replicated-operator` services when you update the configuration files.
