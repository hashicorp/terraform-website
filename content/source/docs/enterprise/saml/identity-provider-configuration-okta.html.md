...
1. In Okta's web interface, click on the "Applications" tab and click "Add Application".
    ![Screenshot: Okta's "Add Application" page](./images/sso-okta-add-application.png)
2. Click "Create New App" on the right hand side of the "Add Application" page.
    ![Screenshot: Okta's "Add Application Selection" page](./images/sso-okta-add-application.png)
3. Select "Web" as the platform and "SAML 2.0" as the sign on method, then click "Create".
    ![Screenshot: Okta's modal dialog for beginning a new application](./images/sso-okta-new-application-form.png)
4. In the "General Settings" page, enter an app name, then click "Next".
...
5. In the "Configure SAML" page, configure the following settings with the specified values:
    ...
    ![Screenshot: The "Create SAML Integration" page of Okta's new app workflow, with the specified settings entered.](./images/sso-okta-new-application-saml.png)

6. Still in the "Create SAML Integration" page, configure a group attribute statement to report which teams a user belongs to. Under the "Group Attribute Statements (Optional)" header, configure the statement as follows:
    ...
    - **Filter:** A filter type and filter value that will match all of the relevant groups that each user belongs to. The exact filter expression depends on how your Okta groups are configured, and which subset of groups you want to expose to TFE. Note that TFE ignores group names that do not correspond to existing TFE teams; see [Team Membership Mapping](./team-membership.html) for more details.
      ...
      ![Screenshot: Further down on Okta's "Create SAML Integration" page, with a group attribute statement partially configured.](./images/sso-okta-new-application-attributes.png)
7. Still in the "Create SAML Integration" page, optionally configure a site admin permissions attribute statement. This statement determines which users can administer the entire Terraform Enterprise instance (see [Administering Terraform Enterprise](../admin/index.html) for more information about site admin permissions). Under the "Attribute Statements (Optional)" header, configure a statement as follows:
...
8. Preview the SAML response and make sure it matches your expectations.
9. Finish configuring the SAML app in Okta, then copy the provided endpoint URLs and certificate to your Terraform Enterprise SAML settings at `https://<TFE_HOSTNAME>/app/admin/saml`. TFE requires a single sign-on URL, a single log-out URL, and a PEM (base64) encoded X.509 certificate.
10. In the application configuration page, in the "Sign On" tab, click "View Setup Instructions". A new tab will open with SAML2.0 configuration information required for the TFE SAML configuration page.
    ![Screenshot: Okta's TFE application configuration page](./images/sso-okta-view-setup-instructions.png)
11. From the tab that opens, copy the "Identity Provider Single Sign-On URL" into the "Single Sign-On URL" field in the TFE SAML configuration page under "Identity Provider Settings".
    ![Screenshot: Okta's application SAML configuration page](./images/sso-okta-saml-configuration-information.png)
12. Ensure the users and groups are administered correctly in Okta and the TFE application created and configured above is assigned to the correct users and/or groups.
