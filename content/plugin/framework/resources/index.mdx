...
1. Using the configured API client injected into the resource by the resource
   type's `NewResource` method
1. Calling the [`State.RemoveResource`
...
`ImportState` creates an initial Terraform state to bring the resource under management via the [`terraform import` command](/cli/commands/import). This method must provide enough state for the resource to be successfully refreshed via the `Read` method. This is usually accomplished by:
...
Additional documentation and code examples are available in the [Resource Import](/plugin/framework/resources/import) page.
...
- [Import](/plugin/framework/resources/import) helps practitioners bring existing infrastructure under Terraform's management.
