...
In SDKv2, resources and data sources automatically included an implicit, root level `id` attribute. The `id` attribute is no longer implicitly added.
...
To workaround this limitation of the acceptance testing framework, add a root level `id` attribute to resources and data source schemas and ensure the attribute value is appropriately [written to state](/plugin/framework/writing-state). Conventionally, `id` is a computed attribute which contains the identifier for the resource.
