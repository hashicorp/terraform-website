...
happening in a provider as it happens, and then examining that record to diagnose issues. Logs can help you debug your provider without needing access to the environment or configuration.

When developing your provider, think carefully about what information you will need for debugging. You must inject log lines into your provider during development because they cannot be inserted into the binary after it has been built. You will need to add a new log line and recompile your provider for new information to be surfaced in log output.
