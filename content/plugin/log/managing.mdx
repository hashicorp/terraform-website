...
Terraform, providers, and provider SDKs can all write logs that provide insight into operations and help you diagnose bugs. You can use environment variables to turn on logging and filter log output. You can also set the log format and output path.

## Enable Logging

Logging is off for all subsystems by default. Set one or more logging environment variables to either [`JSON` format](#log-format) or a [verbosity level](#log-levels) to write logs for a Terraform run. To write:

- **All logs** for Terraform binary, all providers, and all provider SDKs, set `TF_LOG`. We recommend setting `TF_LOG=TRACE` when you report bugs to issue trackers.
- **A subset of logs** (e.g., for a single provider), set the only the environment variables for your use case.

Some environment variables take precedence over others. For example, `TF_LOG` takes precedence over all other logging variables. So if you set `TF_LOG=TRACE` and then `TF_LOG_PROVIDER_AZUREM=WARN`, all providers will write logs at the `TRACE` level.
...
To specify logging verbosity, set a [logging environment variable](#environment-variables) to one of the following values. The values are listed in order of least verbose to most verbose:
