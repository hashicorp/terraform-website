...
Terraform, providers, and provider SDKs can all write logs that provide insight into operations and help you diagnose bugs. This page explains how to turn on logging, filter log output, and manage log format.

## Logging Environment Variables

Logging is off for all subsystems by default. Set one or more of the the logging environment variables below to either [`JSON` format](#log-format) or a [verbosity level](#logging-levels) to write logs for a Terraform run.
...
During testing, Terraform ignores `TF_LOG_CORE`, and logging environment variables only control the log output for the provider under test. The one exception is `TF_LOG_PROVIDER`, which does not affect provider or provider SDK logs during acceptance tests.
