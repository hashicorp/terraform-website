...
Terraform, provider SDKs, and providers can all write logs that provide insight into operations and can help you diagnose bugs. This page explains how to turn on logging for one or more of these subsystems and manage logging output.


## Logging Environment Variables

Logging is off for all subsystems by default. To write log data for a Terraform run, set one or more of the logging environment variables below to either [`JSON` format](#log-format) or a [verbosity level](#logging-levels).
...
| Variable Name | Controls logs for | Precedence and Usage Notes |
| --------------|-------------|-------------|
`TF_LOG` | Terraform, the SDKs, and all providers | Overrides all other logging environment variables. |
| `TF_LOG_CORE` | Terraform binary | Does not include providers and only works if `TF_LOG` is not set.|
| `TF_LOG_PROVIDER` | All providers used during the run | Overrides logging environment variables for specific providers and provider SDKs (e.g., `TF_LOG_PROTO`).|
| `TF_LOG_PROVIDER_{PROVIDER_NAME}`, (e.g., `TF_LOG_PROVIDER_AWS`)| Single provider  | Only applies to providers that write logs using the `tflog` package. Does not affect logs written by legacy `log.Print*` functions. |
|`TF_LOG_SDK` | Provider SDKs for all providers used in the run | As it only applies to logs written with `terraform-plugin-log`, this variable does not yet apply to a large number of logs written by SDKv2. You can control those logs with `TF_LOG` or `TF_LOG_PROVIDER`. |
`TF_LOG_SDK_PROTO` | [`terraform-plugin-go`](https://pkg.go.dev/github.com/hashicorp/terraform-plugin-go) | Overrides `TF_LOG_SDK` for `terraform-plugin-go` logs. |
| `TF_LOG_SDK_HELPER_SCHEMA`| `helper/schema` package in [`terraform-plugin-sdk/v2`](https://pkg.go.dev/github.com/hashicorp/terraform-plugin-sdk/v2/helper/schema)  | Overrides `TF_LOG_SDK` for `terraform-plugin-sdk/v2` logs. As it only applies to logs written with `terraform-plugin-log`, this variable does not yet apply to a large number of logs written by SDKv2. You can control those logs with `TF_LOG` or `TF_LOG_PROVIDER`. |
|`TF_LOG_SDK_MUX`| [`terraform-plugin-mux`](https://pkg.go.dev/github.com/hashicorp/terraform-plugin-mux) module | Overrides `TF_LOG_SDK` for `terraform-plugin-mux` logs.

## Logging Levels

Set [logging environment variables](#logging-environment-variables) to one of the following values to set its associated loggers to a specific output verbosity level.

```
TF_LOG_PROVIDER=WARN
```

The values are listed in order of least verbose to most verbose:
