...
Terraform, providers, and provider SDKs can all write logs that provide insight into operations and can help you diagnose bugs. Logging is off for all of these subsystems by default, but you can use logging environment variables to turn on logging and filter log output. You can also add custom metadata to the log output for a particular request.

## Logging Environment Variables

To write log data for a Terraform run, set one or more of the logging environment variables below to either [`JSON` format](#log-format) or a [verbosity level](#logging-levels). Use `TF_LOG` to write all available logs from Terraform binary, all providers and all provider SDKs. To write only a subset of logs (e.g., for a single provider), do not set `TF_LOG` and instead set specific logging environment variables for your use case.
...
Explicitly setting the log format to `JSON` will output logs in a machine-readable format at the TRACE logging level. The format is not guaranteed to be stable between releases.
...
During testing, Terraform ignores `TF_LOG_CORE`, and logging environment variables only control the log output for the provider under test.

Refer to the [acceptance testing](/plugin/sdkv2/testing/acceptance-tests) section for more information about additional environment variables that are available.
