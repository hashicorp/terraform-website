...
| Variable | Logs Written By | Precedence and Usage Notes |
| --------------|-------------|-------------|
...
|`TF_LOG_SDK` | Provider SDKs for all providers used in the run | Only applies to logs written with `terraform-plugin-log` and does not yet apply to a large number of SDKv2 logs. For remaining logs use `TF_LOG` or `TF_LOG_PROVIDER`. |
...
| `TF_LOG_SDK_HELPER_SCHEMA`| [`terraform-plugin-sdk/v2`](https://pkg.go.dev/github.com/hashicorp/terraform-plugin-sdk/v2/helper/schema) `helper/schema` package | Overrides `TF_LOG_SDK` for `terraform-plugin-sdk/v2` logs. Only applies to logs written with `terraform-plugin-log` and does not yet apply to a large number of SDKv2 logs. For remaining logs use `TF_LOG` or `TF_LOG_PROVIDER`. |
|`TF_LOG_SDK_MUX`| [`terraform-plugin-mux`](https://pkg.go.dev/github.com/hashicorp/terraform-plugin-mux)| Overrides `TF_LOG_SDK` for `terraform-plugin-mux` logs.

## Logging Levels

To specify logging verbosity, set a [logging environment variable](#logging-environment-variables) to one of the following values.
