...
Terraform, provider SDKs, and providers can all write logs that provide insight into operations and can help diagnose bugs. Use the environment variables on this page to turn on logging for one or more of these subsystems during a Terraform run. For each subsystem, you can specify the log verbosity level according to your use case.
...
The variables are listed in When you report bugs to issue trackers, we recommend setting `TF_LOG=TRACE`.
...
Use the `With()` method to embed request-scoped values with a logger that will be printed in every log output line associated with the request. This lets you more easily filter output data.
...
While testing, Terraform ignores `TF_LOG_CORE`, and logging environment variables only control the log output for the provider under test. Use the following environment variables to change where and how Terraform stores log outputs from tests.
