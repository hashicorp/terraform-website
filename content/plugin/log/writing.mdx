...
Use the [`tflog` package](https://pkg.go.dev/github.com/hashicorp/terraform-plugin-log/tflog) to write log output for your provider. Both the SDK or the framework use `tflog` to let you write logs and display them to users with minimal setup.
...
This page explains how to use `tflog` to write log output at varying verbosity levels, add variables, and designate logs that relate to distinct sections of code (e.g., the API client).
