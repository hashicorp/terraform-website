...
Use variables to attach information to specific logs or to a logger itself, which will be attached to all subsequent logs. Both methodologies can be combined to simplify logging code.
...
Use [`tflog.With()`](https://pkg.go.dev/github.com/hashicorp/terraform-plugin-log/tflog#With) to add variables that will be attached to subsequent logs:
...
In certain situations, it may be preferable to conditionally attach the variables. Instead of overwriting the logger context, a new one can be created:
...
Refer to the [Subsystems](#subsystems) section for another methodology to manage larger sets of similar loggers.
...
ctx = tflog.NewSubsystem(ctx, "my-subsystem", tflog.WithLevelFromEnv("TF_LOG_PROVIDER_MYPROVIDER_CLIENT"))
```
