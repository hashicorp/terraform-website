...
To increase the number of concurrent runs, adjust the `capacity_concurrency` setting. Note that this setting is not limited by
system checks; it depends on the operator to provide enough memory to the system to accommodate the requested
concurrent capacity. For instance, if `capacity_concurrency` is set to `100`, the instance would require, at a minimum,
52 GB of memory just for Terraform runs, if the worker memory was set to 512. The rest of the TFE application requires 4 GB of memory at a minimum, on top of the Operating System requirements.
...
The default setting of `0` allows a Terraform run to utilize all available CPU cores on the TFE instance. You can adjust this setting by inputting a specific limit to the number of CPU cores the Terraform runs will have access to. 
...
but in general we suggest a minimum of 50 IOPS per concurrent Terraform run. So if an instance is configured for
10 concurrent runs, the disk should have 500 IOPS allocated. For reference, on AWS, an EBS volume
with an allocated size of 250 GB comes with a steady state of 750 IOPS.

It is recommended to use a disk with a minimum of 500 IOPS, but high load systems should consider increasing this significantly. A production instance, with a consistantly high level of utilization, and a concurrency of 10, should have a disk with around 3,000 IOPS, ideally. 
