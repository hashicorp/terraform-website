...
1. Prepare the machine that will run Terraform.
1. Prepare some required GCP infrastructure.
1. Write a Terraform configuration that calls the deployment module.
1. Apply the configuration to deploy the cluster.
...
Before you begin, follow the [Pre-Install Checklist](/enterprise/before-installing) and ensure you have all of the prerequisites. This checklist includes several important decisions, some supporting infrastructure, and necessary credentials.
...
* The system running Terraform can authenticate to GCP. For more details, see [Google Provider Configuration Reference: Credentials](https://registry.terraform.io/providers/hashicorp/google/latest/docs/guides/provider_reference#credentials-1).
* You're familiar enough with Terraform to write simple configurations that call [modules ](/language/configuration-0-11/modules) and specify [output values](/language/configuration-0-11/outputs).
...
* A Google Cloud SQL PostgreSQL instance - please see [PostgreSQL Requirements](/enterprise/before-installing/data-storage-requirements/postgres-requirements).
...
1. Review the module's [input variables][inputs].
1. Create a new Terraform configuration that calls the `hashicorp/terraform-enterprise/google` module:
   ...
   * Fill in any optional variables as desired. If you omit all optional variables, the module will deploy a mid-sized cluster using the _Demo_ operational mode.
...
4. Initialize Terraform and run a plan. If you are running Terraform from the CLI, you can do this by navigating to the configuration's directory and running:
   ...
   ```
4. If the plan runs without errors and looks correct, apply it:
...
4. Once the apply has finished, Terraform will display any root-level outputs you configured. For example:
...
4. Open the installer dashboard in your web browser, and log in with the password specified in the `installer_dashboard_password` output. Follow the instructions at [Terraform Enterprise Configuration](/enterprise/install/interactive/config) to finish setting up the application.
