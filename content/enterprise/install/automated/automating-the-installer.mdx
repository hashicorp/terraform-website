...
This file contains the values you would normally provide in the settings screen. You need to create this file first since it is referenced in the `ImportSettingsFrom` property in `/etc/replicated.conf`, which will be described below.

### Format

The settings file is JSON formatted. All values must be strings.  The example below is suitable for a _Mounted Disk_ installation:
...
```json
{
    ...
    "disk_path": {
        "value": "/var/lib/terraform-enterprise"
    },
    "enc_password": {
        "value": "CHANGEME"
    }
...
Here is an example `app-config export` output for an instance configured in _Mounted Disk_ mode:
...
- `production_type` — One of `external` or `disk`. Defaults to `disk`.
