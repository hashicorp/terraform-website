...
This file contains the values you would normally provide in the settings screen, which may be as simple as choosing the _Demo_ installation type or as complex as specifying the PostgreSQL connection string and S3 bucket credentials and parameters. You need to create this file first since it is referenced in the `ImportSettingsFrom` property in `/etc/replicated.conf`, which will be described below.

### Format

The settings file is JSON formatted. All values must be strings.  The example below is suitable for a _Demo_ installation:
...
Here is an example `app-config export` output for an instance configured in _Demo_ mode:
...
Note the `private-address` and `public-address` flags provided to the installer. We recommend you always supply these flags. If the virtual machine instance will not have its own separate public IP address (i.e. you are using a load balancer and a private subnet), you should provide the private IP address for both flags.

If these flags are left out the installer will attempt to determine them itself and will prompt for them if it is unable to. If using cloud-init or another non-interactive method to run the installer you will see this prompt in the log output, but the installer will continue to run. If the installer is unable to determine the IP addresses and you do not provide them via the flags, it will appear that the install has been successful, but functionality of Terraform Enterprise will be degraded. For example you may see a `Internal error: SIW-001` error when importing modules to the private registry or when you run the `tfe-admin health-check` command you may see that the Archivist and Vault containers are unhealthy. 
