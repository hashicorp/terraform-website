...
Terraform Enterprise now supports scaling up to three nodes as part of the Active/Active deployment. Scaling beyond two nodes requires careful evaluation and scaling of the external services as well, particularly the database server. Scaling to three nodes does not alter the [upgrade requirement](https://www.terraform.io/enterprise/admin/infrastructure/active-active#upgrading-tfe-or-patching-tfe-node-instances) that you scale down to a single node, or rebuild entirely.

#### PostgreSQL Server

The Terraform Enterprise PostgreSQL server will typically hit the CPU capacity before other resources. Close monitoring of the CPU while in a two node configuration should be done prior to scaling up to three nodes. Additionally, the database maximum connection count may need to be manually modified to allow for the additional load. The defaults vary by cloud, so please refer to the documentation for the cloud your installation is on. 
...
Rarely, certain types of workloads may cause spikes in the Redis server CPU or memory. We recommend the Redis server be monitored and scaled up as necessary. 
...
As you scale out Terraform Enterprise beyond two nodes, you may be adding additional stress to your network and dramatically increasing the number of API calls made in your cloud account. Each of the three support clouds have their own default limits and processes by which those limits might possibly be increased. 
...
Please consult the documentation and support of your cloud platform for more information on any of these topics.

#### Consider Agents

An alternative to scaling Terraform Enterprise beyond two, or even three, nodes is to utilize [Terraform Cloud Agents](https://www.terraform.io/enterprise/admin/agents-on-tfe). When agents are utilized, the actual Terraform plans and applies will take place on the target system that has the agent executable installed, with a much smaller impact on the Terraform Enterprise servers than local runs. Agents can run in other regions, other clouds, and even private cloud, provided the network connectivity is configured to allow for it. 
