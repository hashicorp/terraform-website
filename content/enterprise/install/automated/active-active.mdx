...
Depending on your infrastructure and Terraform Enterprise configuration, you may need to configure your application gateway or load balancer for "sticky sessions." Sticky session is the common term for a setting in a load balancer or gateway device that ensures traffic is routed back to the original system that initiated a given request. For example, an Active/Active deployment on Azure with SAML authentication requires sticky sessions be enabled to ensure the authentication with the SAML server is successful. The terminology for this varies across clouds. Please reference the documentation for your infrastructure.
