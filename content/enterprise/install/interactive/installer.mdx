...
-> **Important:** Before you begin, consult the [Pre-Install Checklist](/enterprise/before-installing) for prerequisites. You'll need to prepare data files and a Linux instance.
...
Unless the execution mode of a workspace is set to "local", Terraform Enterprise performs [remote operations](/cloud-docs/run#remote-operations), running Terraform in its own worker VMs.
...
Terraform Enterprise uses the `HIGH:!aNULL:!MD5` ciphersuites by default. If necessary, a custom ciphersuite can be provided:
...
1. _Optional:_ Configure the product to use an externally managed Vault cluster.
   See [Externally Managed Vault Cluster](/enterprise/before-installing/data-storage-requirements/vault) for details about the required Vault configuration before using this option.
