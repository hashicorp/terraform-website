...
You can use Terraform Enterprise's instance profile to provide default credentials to workspaces. Configure the [instance metadata options](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/configuring-instance-metadata-options.html) and specify the PUT response hop limit value as 2 for instances using IMDSv2. Terraform will attempt to use the instance profile to provision resources when you do not set credentials as environment variables. However, this approach presents a few security risks:
