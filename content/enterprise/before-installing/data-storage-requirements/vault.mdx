...
1. Input the address of the vault cluster, role_id, and secret_id during the
   install process.
...
### External Vault to Internal Vault Migration

When migrating from an externally managed Vault service to the provided Terraform Enterprise internal Vault service, the backup process requires additional permissions to retreive keys from certain paths. To accomplish this the Vault policy needs to be modified to provided read access to paths. 
