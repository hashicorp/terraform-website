---
page_title: Infrastructure Administration - Terraform Enterprise
---

# Terraform Enterprise Admin CLI commands

The [Active/Active](/enterprise/install/automated/active-active) operational mode disables the Replicated Admin Console. Instead, it provides admin commands to change the configuration, stop the application safely, and produce support bundles. You must use SSH to log in to a node in the Active/Active cluster to run these commands from the command line.

-> **Note**: Admin commands are available on the standalone installation in Terraform Enterprise version v202203-1 and later.
...
This command generates a support bundle for all nodes. The output support bundle file is generated under `/var/lib/replicated/support-bundles`. 
