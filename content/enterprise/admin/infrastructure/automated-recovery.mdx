...
In _Demo_ mode, both application data and installer data are
stored on the Terraform Enterprise instance. In _Mounted Disk_ and _External Services_ modes, only
...
on the mode of operation you're using. If you're in _Demo_ mode,
one hour is recommended as that will minimize the data loss to one
hour only. For _Mounted Disk_ or _External Services_, Daily is recommended
