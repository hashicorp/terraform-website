...
1. The minimum required Replicated version is now 2.53.4. Airgap customers can visit this URL to download the latest Replicated version: [https://install.terraform.io/airgap/latest.tar.gz](https://install.terraform.io/airgap/latest.tar.gz).
...
2. Improved the performance and reliability when destroying workspaces.
3. Changed documentation link on preparing a module repository.
4. Private providers are now supported in the private registry.
5. Added API endpoints for [downloading configuration versions](https://www.terraform.io/cloud-docs/api-docs/configuration-versions#download-configuration-files).
1. Added [Variable Sets](https://www.terraform.io/cloud-docs/api-docs/variable-sets).
1. Added html and/or plain text versions to all account-related emails.

## APPLICATION LEVEL BUG FIXES:

1. Fixed response code for JSON parse errors.
...
1. Ongoing container updates to address reported vulnerabilities in underlying packages / dependencies, including OpenSSL-related [CVE-2022-0778](https://nvd.nist.gov/vuln/detail/CVE-2022-0778).
