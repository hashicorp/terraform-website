...
| `metadata.protocol_versions` | List of strings with supported Terraform protocol versions of the provider. Providers developed using [Terraform Plugin SDK version 2](/plugin/sdkv2) should set this to `["5.0"]`. Providers developed using [Terraform Plugin Framework](/plugin/framework) should set this to `["6.0"]`. Defaults to `["5.0"]`. |
