...
description: Learn how to release a provider and publish it to the Terraform Registry.
---
...
Your provider should contain an overview document (index.md), as well as a doc for each resource and data-source. See [Documenting Providers](/registry/providers/docs) for details about generating documentation and how to ensure your provider documentation renders properly on the Terraform Registry.
...
| `metadata.protocol_versions` | List of strings with supported Terraform protocol versions of the provider. Providers developed using [Terraform Plugin SDK version 2](/plugin/sdkv2/sdkv2-intro) should set this to `["5.0"]`. Providers developed using [Terraform Plugin Framework](/plugin/framework) should set this to `["6.0"]`. Defaults to `["5.0"]`. |
| `version`                    | Numeric version of the manifest format, not the version of your provider. Set this to `1`.                                                                                                                                                                                                                                     |

~> **Important:** If developing a provider using [Terraform Plugin Framework](/plugin/framework), ensure the protocol versions are set to `["6.0"]` or practitioners will receive errors when attempting to use the provider.
...
We have a list of [recommend OS / architecture combinations](/registry/providers/os-arch) for which we suggest most providers create binaries.
