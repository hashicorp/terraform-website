...
input_files.each do |input_file|
  ...
  url_encoded_filename = input_file.split('/').map { |s| ERB::Util.url_encode(s) }.join('/')
  url_string = url_encoded_filename.sub(site_root_paths, 'http://localhost:4567/').sub(page_extensions, '.html')
