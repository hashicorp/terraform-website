...
By default, Terraform Cloud will automatically mark a run as completed if the plan contains no changes. However, plans that contain no changes can be applied if you create a run in [Allow Empty Apply Mode](/cloud-docs/run/modes-and-options#allow-empty-apply). This is particularly useful for upgrading between Terraform versions in your workspace, [read more on upgrading workspace state.](/cloud-docs/workspaces/state#upgrading-state).  
