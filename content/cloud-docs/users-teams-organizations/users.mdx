...
User accounts belong to individual people. They belong to teams, which are granted permissions on an organization’s workspaces. A user can be a member of multiple [organizations][].

-> **API:** See the [Account API](/cloud-docs/api-docs/account).
...
- For the SaaS version of Terraform Cloud, create a new account at <https://app.terraform.io/signup/account>.
...
To join an organization, you must be invited by an owner of that organization. In order to have any permissions with that organization, an owner will need to add you to one or more teams. For more information, see [Organization Settings: Users][invite] and [Managing Team Membership](/cloud-docs/users-teams-organizations/teams#managing-team-membership).
...
For more information, see [Administering Terraform Enterprise](/enterprise/admin).
...
![The user settings menu item, in the upper right menu.](/img/docs/user-settings.png)
...
-> **Note:** You can't leave an organization if you are the last member of its owners team. You can either add a new owner and then leave, or you can [delete the organization](/cloud-docs/users-teams-organizations/organizations#general).
...
Password management isn't available if your Terraform Enterprise instance uses [SAML single sign on](/enterprise/user-management/saml).
...
For more details, see [Two-Factor Authentication](/cloud-docs/users-teams-organizations/2fa).
...
- Authenticating with the [Terraform `remote` backend](/language/settings/backends/remote), which requires a token in the CLI config file or in the backend configuration.
- Using [private modules](/cloud-docs/registry/using) in command-line Terraform runs on local machines requires [a token in the CLI config file](/cloud-docs/registry/using#authentication).

Terraform Cloud has three kinds of API tokens: user, team, and organization. For more information about team and organization tokens, see [API Tokens](/cloud-docs/users-teams-organizations/api-tokens).

Protect your tokens carefully, because they can do anything your user account can. For example, if you belong to a team with permission to read and write variables for a workspace, your API token can edit variables in that workspace. ([More about permissions.](/cloud-docs/users-teams-organizations/permissions))

[permissions-citation]: #intentionally-unused---keep-for-maintainers

Since users can be members of multiple organizations, user tokens work with any organization their user belongs to.

![The user tokens page](/img/docs/user-tokens.png)
