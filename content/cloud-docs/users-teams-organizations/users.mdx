...
description: >-
  How to create an account, change user settings, set up two-factor
  authentication, create user API tokens, and more.
---
...
-> **API:** Refer to the [Account API](/cloud-docs/api-docs/account).
...
- **Sign-Up Page:** For Terraform Cloud, go to [https://app.terraform.io/signup/account](https://app.terraform.io/signup/account). For Terraform Enterprise, go to `https://<TFE HOSTNAME>/signup/account`. After you create an account, you will not belong to any organizations. To begin using Terraform Cloud, you can either [create an organization](/cloud-docs/users-teams-organizations/organizations#creating-organizations) or ask an organization owner to send you an invitation email.

## Joining Organizations and Teams

An organization owner must [invite you to join an organization](/cloud-docs/users-teams-organizations/organizations#users). After you join, the organization owner must also [add you to one or more teams](/cloud-docs/users-teams-organizations/teams#managing-team-membership).

[permissions-citation]: #intentionally-unused---keep-for-maintainers

Terraform Cloud sends user invitations are sent by email. If the invited email address matches an existing Terraform Cloud account, the invitee can join the organization with that account. Otherwise, they must create a new account to work with that organization.

## Site Admin Permissions

On Terraform Enterprise instances, some user accounts have a special site admin permission that allows them to administrate the entire instance.

Admin permissions are distinct from normal organization-level permissions, and they apply to a different set of UI controls and API endpoints. Although admin users can administer any resource across the instance when using the site admin pages or the [admin API](/cloud-docs/api-docs/admin), they have a normal user's permissions (with access determined by the teams they belong to) when using an organization's standard UI controls and API endpoints.

Refer to [Administering Terraform Enterprise](/enterprise/admin) for more details.

## User Settings

To view your settings page, click your user icon in the upper right corner and select **User settings** from the menu. Your **Profile** page appears, showing your username, email address, and avatar.

### Profile

Terraform Cloud user profiles include a username and an email address. You can edit these from the **Profile** page within **User Settings**.
...
- Authenticating with the [Terraform Cloud CLI integration](/cli/cloud/settings) or the [`remote` backend](/language/settings/backends/remote). These require a token in the CLI config file or in the backend configuration.
- Using [private modules](/cloud-docs/registry/using) in command-line runs on local machines. This requires [a token in the CLI config file](/cloud-docs/registry/using#authentication).

Protect your tokens carefully, because they can do anything your user account can. For example, if you belong to a team with permission to read and write variables for a workspace, your API token can edit variables in that workspace. Refer to [permissions.](/cloud-docs/users-teams-organizations/permissions) for more details.
...
To revoke a token, click the 🗑 **trash can** button next to it. That token will no longer be able to authenticate as your user account.
