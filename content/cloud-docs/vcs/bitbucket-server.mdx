...
1. Click "Continue", which will allow you to use the **Consumer Key** and
**Public/Private Keys** when creating a new Application Link in BitBucket
Server.

**If you have your own _Consumer Key_ and _Public Key_, then skip to step B.
Otherwise, continue to step A to use the pre-generated keys.**

A) Leave this page open in a browser tab. In the next step, you will copy and paste the unique **Consumer Key** and **Public Key.**
...
B) Insert the **Consumer Key**, **Public Key**, and **Private Key** into this
form that matches the Application Link that already exists.

   ![Terraform Cloud screenshot: Consumer key and public key](/img/docs/bitbucket-server-tfe-custom-consumer-and-public-key.png)
