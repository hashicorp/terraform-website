...
# Using Providers and Modules from the Private Registry (link)
...
- **Private Provider and Private Module `source` strings:** Private providers and modules use a [different format](/cloud-docs/registry/using#using-providers-and-modules-in-configurations).
...
- **Authentication:** If you run Terraform on the command line, you must [authenticate](/cloud-docs/registry/using#authentication) to Terraform Cloud or your Terraform Enterprise instance to use modules and providers in your organization’s private registry.
...
The syntax for referencing public modules in the [module block](/language/modules/syntax) `source` argument is `<NAMESPACE>/<MODULE NAME>/<PROVIDER_NAME>`.
...
The syntax for referencing private modules in the [module block](/language/modules/syntax) `source` argument is `<HOSTNAME>/<ORGANIZATION>/<MODULE NAME>/<PROVIDER_NAME>`.
...
To get started, copy and paste the usage example on the module's details page.
...
Terraform version 0.11 or later is required to use private modules in Terraform cloud workspaces and use the CLI to apply configurations with private modules. Terraform version 0.13 and later is required to use private providers in Terraform Cloud workspaces and apply configurations with private providers.
