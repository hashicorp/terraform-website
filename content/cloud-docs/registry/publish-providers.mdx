...
1. Use the [Add a GPG key endpoint](/cloud-docs/api-docs/providers#add-a-gpg-key-private-provider) to add the public key that matches the signing key for the release. The response contains a `key-id` that you will use to create a provider version.

    ```json
    "key-id": "32966F3FB5AC1129"
    ```
1. Use the [Create a Provider Version endpoint](/cloud-docs/api-docs/providers#create-a-provider-version-private-provider) to create a version with the `key-id` from the previous step. The response includes URL links that you will use to upload the `SHA256SUMS` and `SHA256.sig` files.
...
Use the [Registry Providers API](/cloud-docs/api-docs/providers) to create, read, update, and delete the following:
...
In addition to the [Registry Providers API](/cloud-docs/api-docs/providers), you can delete providers or provider versions through the Terraform Cloud API. To delete providers and versions:
