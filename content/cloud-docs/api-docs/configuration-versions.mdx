...
A configuration version created through the API can only be used in runs if it is in an `uploaded` state. A configuration version created through a linked VCS repository may also be used in runs if it is in an `archived` state.
...
| `fetching`             | For configuration versions created via a commit to a connected VCS repository, this state indicates that Terraform Cloud is in the process of fetching the associated files from VCS.                                                                                    |
| `uploaded`             | The configuration version is fully processed and ready for use in runs.                                                                                                                                                                                                  |
| `archived`             | All immediate runs are complete and the files associated with this configuration version have been discarded. If the configuration version was created via a connected VCS repository, it may be used in new runs, and Terraform Cloud will re-fetch the files from VCS. |
| `errored`              | Something went wrong when processing this configuration version. Errored configuration versions cannot be used to create new runs.                                                                                                                                       |
...
Notifies Terraform Cloud to discard the uploaded `.tar.gz` file associated with this configuration version.

Only configuration versions created via the API or CLI, in an `uploaded` state, and with no associated runs in progress can be archived using this endpoint. Otherwise, calling this endpoint will result in an error.

VCS configuration versions are archived automatically once their runs are complete, then re-fetched for subsequent runs.

| Status  | Response                  | Reason(s)                                                                                                                                     |
| ------- | ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| [202][] | none                      | Successfully initiated the archive process.                                                                                                   |
| [409][] | [JSON API error object][] | Configuration version was in a non-archivable state, or the configuration version was created via VCS and cannot be archived through the API. |
