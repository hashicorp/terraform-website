...
| `data.attributes.plan-only`                        | bool                 | (from configuration version)                       | Specifies if this is a speculative, plan-only run that cannot be applied. Often used in conjunction with terraform-version in order to test whether an upgrade would succeed.                                                              |
...
| `data.attributes.allow-empty-apply`                | bool                 |                                                    | Specifies whether Terraform can apply the run even when the plan contains no changes. Often used to upgrade state after upgrading a workspace to a new terraform version.                                          |
