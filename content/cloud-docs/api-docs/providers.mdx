...
Uploads a GPG Key to a private registry scoped with a namespace. The response will provide a "key-id" which is needed to [Create a Provider Version](#create-a-provider-version) (see below).
...
Updates the GPG key. Only the `namespace` attribute is allowed to be updated and has to match a namespace/organization the User has access to.
...
| `:os`                | The os of the provider platform that is being deleted.                                                                                                                                                               |
