...
#### Private Providers
When created, the private provider record will be available in the organization's registry provider list. After creating a private provider, a version must be created and uploaded in order to be usable. Providers created this way do not automatically update with new versions; instead, you must explicitly create and upload each new version with the [Create a Provider Version](#create-a-provider-version) endpoint.
