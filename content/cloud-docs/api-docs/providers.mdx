...
## Create a GPG Key (Not sure if this goes here or in registry-api docs)
...
curl \
  ...
  --request POST \
  --data @payload.json \
  https://app.terraform.io/api/v2/organizations/demo-custom-providers/registry-providers/private/demo-custom-providers/random/versions
```

### Sample Response

```json
<sample_response_here>
```
...
## Get Terraform Request (private provider)

`GET /organizations/:organization_name/registry-providers/:registry_name/:namespace/:name/versions/:version/download/:os/:architecture`
