...
Lists the public providers included in the private registry for the specified organization. 
...
Returns a list of GPG keys that have been added to a private registry. Requires a filter by namespace(s).
...
| Parameter            | Description                                                                                                                                                           |
| -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `filter[field name]` | **Required.** Restricts results to those with the matching field name value. A valid value is `namespace` and can contain multiples (`filter[namespace]=org1, org2`). |
...
curl \
  --request GET \
  --header "Authorization: Bearer $TOKEN" \
  https://app.terraform.io/api/registry/private/v2/gpg-keys?filter[namespace]=hashicorp
```
...
```json
{
  ...
  "links": {
    "first": "/v2/gpg-keys?filter%5Bnamespace%5D=hashicorp&page%5Bnumber%5D=1&page%5Bsize%5D=15",
    "last": "/v2/gpg-keys?filter%5Bnamespace%5D=hashicorp&page%5Bnumber%5D=1&page%5Bsize%5D=15",
    "next": null,
    "prev": null
  },
  "meta": {
...
## Update a GPG Key (Private Provider)
...
## Delete a GPG Key (Private Provider)
...
## Create a Provider Version (Private Provider)
...
## Get a Version (Private Provider)
...
| `data.attributes.arch`     | string |          | A valid operating system string.                       |
...
**Note:** `provider-binary-uploaded` will be `false` if that file hasn't been uploaded to Archivist yet. In this case, instead of including a link to `provider-binary-download`, the response will include an upload link `provider-binary-upload`.

## Get a Platform (Private Provider)
...
## Delete a Platform (Private Provider)
