...
A workspace's state version can be upgraded to a new Terraform version without making any configuration changes using no-operation (empty) applies. Empty applies are a [type of run mode](/cloud-docs/run/modes-and-options) that instructs Terraform Cloud to perform an apply when a plan produces no changes.   
