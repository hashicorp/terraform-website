...
After you create a new workspace from a version control respository, Terraform Cloud scans its configuration files for undefined [Terraform variables](/cloud-docs/workspaces/variables#terraform-variables) and automatically creates those variables within the workspace. Terraform cannot perform successful runs until you set values for all undefined variables.

Choose one of the following actions:

- To skip this step, click **Go to workspace overview**. Terraform Cloud still adds all Terraform variables to the workspace with undefined values. You can set values for these variables later from within the workspace.
