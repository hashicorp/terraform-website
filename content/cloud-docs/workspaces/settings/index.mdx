...
Every workspace has a unique ID that cannot be changed. Workspace IDs are sometimes necessary when working with [Terraform Cloud's API](/cloud-docs/api-docs).
...
To disable remote execution for a workspace, change its execution mode to "Local". This mode lets you perform Terraform runs locally with the [CLI-driven run workflow](/cloud-docs/run/cli). The workspace will store state and variables, which Terraform can access with the [CLI integration](/cli/cloud).
...
1. Disable [auto-apply](/cloud-docs/workspaces/settings#auto-apply-and-manual-apply) if you have it enabled.
...
1. Enable [auto-apply](/cloud-docs/workspaces/settings#auto-apply-and-manual-apply), if you had it enabled before changing your execution mode.
...
- Plans queued by users without permission to apply runs for the workspace must be approved by a user who does have permission. ([More about permissions.](/cloud-docs/users-teams-organizations/permissions))
- Plans queued due to [run triggers](/cloud-docs/workspaces/settings/run-triggers) from another workspace must always be manually applied.
...
In [runs triggered by VCS commits](/cloud-docs/run/ui), this is automatic. In [CLI-driven runs](/cloud-docs/run/cli), Terraform's CLI sometimes uploads additional content:
...
~> **Important:** Unlike other settings, locks can also be managed by users with permission to lock and unlock the workspace. ([More about permissions.](/cloud-docs/users-teams-organizations/permissions))
...
See [Run Notifications](/cloud-docs/workspaces/settings/notifications) for detailed information about configuring notifications.
...
See [Run Triggers](/cloud-docs/workspaces/settings/run-triggers) for detailed information about configuring run triggers.
...
See [Using SSH Keys for Cloning Modules](/cloud-docs/workspaces/settings/ssh-keys) for detailed information about this page.

## Team Access

The "Team Access" page configures which teams can perform which actions on a workspace.

See [Managing Access to Workspaces](/cloud-docs/workspaces/settings/access) for detailed information.
...
See [VCS Connections](/cloud-docs/workspaces/settings/vcs) for detailed information about this page.
