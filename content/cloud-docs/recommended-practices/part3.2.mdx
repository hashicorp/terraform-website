---
page_title: >-
  Part 3.2: From Semi-Automation to Infrastructure as Code - Terraform
...
If your organization already has a configuration management tool, then it’s time to integrate it with Terraform — you can use [Terraform’s provisioners](/language/resources/provisioners/syntax) to pass control to configuration management after a resource is created. Terraform should handle the infrastructure, and other tools should handle user data and applications.
...
Next, you need an advanced workflow that can scale and delegate responsibilities to many contributors. Continue on to [Part 3.3: How to Move from Infrastructure as Code to Collaborative Infrastructure as Code](/cloud-docs/guides/recommended-practices/part3.3).
