...
Let's look at an example. Say you have a page called `/language/foo` which needs to be moved to `/language/nested/foo`. Additionally, this is a page that has been around for a while and we know there are links into `/language/foo.html` left over from our previous website structure. First, we move the page, then adjust the docs sidenav, in `data/language-navigation.js`. Find the category the page is in, and move it into the appropriate subcategory. Next, we add to `miscRedirectsMap` as such.
...
Next, we run a global search for internal links to `/language/foo`, and make sure to adjust them to be `/language/nested/foo` - this is to ensure that client-side navigation still works correctly. _Adding a redirect alone is not enough_.
